...
def _compile_sqlserver(string):
	...
	for (t, s) in misc.tokenizepi(string):
		if t is None:
			...
			# Treat unknown PIs as text
			foundsql.append((-1, "<?%s %s?>" % (t, s)))

	result = []
	if foundargs:
		result.append("(\n\t%s\n)\n" % u",\n\t".join(foundargs))
	if not foundproc:
		result.append("returns %s\n" % foundtype)
	result.append("as\n")
	result.append("begin\n")
	if not foundproc:
		result.append("\tdeclare @c_out %s;\n" % foundtype)
	...
	for (mode, string) in foundsql:
		if mode == -1:
			...
			result.append("\tset @c_out = @c_out + %s;\n" % string)
		elif mode == 0:
			result.append(string)
			result.append("\n")
		else: # mode == 1
			result.append("\tset @c_out = @c_out + set @c_out = @c_out + convert(varchar, isnull(%s, ''));\n" % string)
	if not foundproc:
...
def compile(string, mode="oracle"):
	"""
	The :class:`unicode` object :var:`string` must be a string containing
	embedded TOXIC processing instructions. :func:`compile` creates the body of
	an Oracle or SQL Server function or procedure from it. :var:`mode` can be
...
def prettify(string, mode="oracle"):
	"""
	Try to fix the indentation of the PL/SQL snippet passed in. :var:`mode` can
