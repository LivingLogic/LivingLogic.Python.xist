...
class ColorAST(_ConstWithValueAST):
	...
	def _sqlsource(self, query:Query) -> templatelib.Template:
		return t"{self._sqlvalue(self.value):q}"
...
AddAST.add_rules(t"{'T1'} <- {[dt.DATE, dt.DATETIME]} + {dt.MONTHDELTA}", oracle=t"vsqlimpl_pkg.add_{'t1'}_months({'s1'}, {'s2'})", postgres=t"vsqlimpl.add_{'t1'}_months({'s1'}, {'s2'})")
AddAST.add_rules(t"{'T2'} <- {dt.MONTHDELTA} + {[dt.DATE, dt.DATETIME]}", oracle=t"vsqlimpl_pkg.add_{'t2'}_months({'s2'}, {'s1'})", postgres=t"vsqlimpl.add_{'t2'}_months({'s2'}, {'s1'})")
...
MulAST.add_rules(t"{dt.NUMBER} <- {NUMBERLIKE} * {NUMBERLIKE}", oracle=t"({'s1'} * {'s2'})", postgres=t"(vsqlimpl.number_{'t1'}({'s1'}) * vsqlimpl.number_{'t2'}({'s2'}))")
...
MulAST.add_rules(t"{'T2'} <- {dt.BOOL} * {TEXT}", oracle=t"vsqlimpl_pkg.mul_int_{'t2'}({'s1'}, {'s2'})", postgres=t"vsqlimpl.mul_int_{'t2'}({'s1'}::int::bigint, {'s2'})")
MulAST.add_rules(t"{'T2'} <- {dt.INT} * {TEXT}", oracle=t"vsqlimpl_pkg.mul_int_{'t2'}({'s1'}, {'s2'})", postgres=t"vsqlimpl.mul_int_{'t2'}({'s1'}, {'s2'})")
MulAST.add_rules(t"{'T1'} <- {TEXT} * {dt.BOOL}", oracle=t"vsqlimpl_pkg.mul_int_{'t1'}({'s2'}, {'s1'})", postgres=t"vsqlimpl.mul_{'t1'}_int({'s1'}, {'s2'}::int::bigint)")
MulAST.add_rules(t"{'T1'} <- {TEXT} * {dt.INT}", oracle=t"vsqlimpl_pkg.mul_int_{'t1'}({'s2'}, {'s1'})", postgres=t"vsqlimpl.mul_{'t1'}_int({'s1'}, {'s2'})")
...
MulAST.add_rules(t"{'T1'} <- {LIST} * {dt.BOOL}", oracle=t"vsqlimpl_pkg.mul_int_{'t1'}({'s2'}, {'s1'})", postgres=t"vsqlimpl.mul_{'t1'}_int({'s1'}, {'s2'}::int::bigint)")
MulAST.add_rules(t"{'T1'} <- {LIST} * {dt.INT}", oracle=t"vsqlimpl_pkg.mul_int_{'t1'}({'s2'}, {'s1'})", postgres=t"vsqlimpl.mul_{'t1'}_int({'s1'}, {'s2'})")
