...
## Copyright 2009-2026 by LivingLogic AG, Bayreuth/Germany
## Copyright 2009-2026 by Walter Dörwald
##
## All Rights Reserved
##
...
###
### Stream classes for various output modes.
###

class NullStream:
	"""
	Output stream that ignores all writes.
	"""
	def write(self, text):
		pass
...
class XMLEscapeStream:
	"""
	Output stream that delegates all writes to an underlying stream, but XML
	escapes everything that gets written.
	"""
...
class Context:
	...
	def __init__(self, globals=None, stream=None):
		...
		self.stream = stream if stream is not None else NullStream()
	...
	def add_builtins(cls):
		...
		if not b:
			...
			b["operator"] = _create_module(
				...
				itemgetter=ItemGetter.ul4_type,
			)
...
class ItemGetter:
	ul4_type = InstantiableType("operator", "itemgetter", "Return a callable object that fetches the given items(s) from its operand.")

	def __init__(self, *items):
		self.itemgetter = operator.itemgetter(*items)
