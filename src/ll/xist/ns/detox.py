...
def xml2py(source):
	...
	lines = [
		"# generated by {0} on {1} UTC".format(__file__, datetime.datetime.utcnow()),
		"",
	...
	def endscope(action):
		if not stack:
			raise SyntaxError("can't end {0} scope: no active scope".format(action or "unnamed"))
		if action and action != stack[-1][0]:
			raise SyntaxError("can't end {0} scope: active scope is: {1} {2}".format(action, stack[-1][0], stack[-1][1]))
		return stack.pop()
	...
	for (t, s) in misc.tokenizepi(source):
		if t is None:
			...
			# ignore output outside of functions
			if stackoutput and stackoutput[-1]:
				lines.append("{0}yield __detox_xmlescape__({1})".format(len(stack)*indent, s))
		elif t == "attrexpr":
			# ignore output outside of functions
			if stackoutput and stackoutput[-1]:
				lines.append("{0}yield __detox_xmlescape__({1})".format(len(stack)*indent, s))
		elif t == "code":
			lines.append("{0}{1}".format(len(stack)*indent, s))
		elif t == "def":
			lines.append("")
			lines.append("{0}def {1}:".format(len(stack)*indent, s))
			stack.append((t, s))
			stackoutput.append(True)
		elif t == "class":
			lines.append("")
			lines.append("{0}class {1}:".format(len(stack)*indent, s))
			stack.append((t, s))
			stackoutput.append(False)
		elif t == "for":
			lines.append("{0}for {1}:".format(len(stack)*indent, s))
			stack.append((t, s))
		elif t == "while":
			lines.append("{0}while {1}:".format(len(stack)*indent, s))
			stack.append((t, s))
		elif t == "if":
			lines.append("{0}if {1}:".format(len(stack)*indent, s))
			stack.append((t, s))
		elif t == "else":
			lines.append("{0}else:".format((len(stack)-1)*indent))
		elif t == "elif":
			lines.append("{0}elif {1}:".format((len(stack)-1)*indent, s))
		elif t == "end":
			scope = endscope(s)
			...
			if stackoutput and stackoutput[-1]:
				...
				lines.append("{0}yield {1!r}".format(len(stack)*indent, s))
	if stack:
		raise SyntaxError("unclosed scopes remaining: {0}".format(", ".join(scope[0] for scope in stack)))
	return "\n".join(lines)
