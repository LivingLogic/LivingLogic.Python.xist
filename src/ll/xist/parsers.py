...
class Parser(object):
	...
	def __init__(self, saxparser=SGMLOPParser, prefixes={}, tidy=False, loc=True, validate=True, encoding=None, registry=None):
		"""
		...
		self.encoding = encoding
		self.registry = (registry if registry is not None else xsc.getregistrystack()[-1])
	...
	def _parseHTML(self, stream, base, sysid, encoding):
		...
		def toxsc(node):
			if node.type == "document_html":
				...
				try:
					newnode = self.registry.create_element_xml(name, html)
					if self.loc:
				...
				newnode = self.registry.create_text(decode(node.content))
				if self.loc:
					newnode.startloc = xsc.Location(sysid=sysid, line=node.lineNo())
			elif node.type == "comment":
				newnode = self.registry.create_comment(decode(node.content))
				if self.loc:
					newnode.startloc = xsc.Location(sysid=sysid, line=node.lineNo())
			else:
				newnode = xsc.Null
