...
def adddtd2xnd(ns, dtd):
	# Appends DTD information from ``dtd`` to the :class:`xnd.Module` object
...
def main(args=None):
	p = argparse.ArgumentParser(description="Convert DTDs to XIST namespace (on stdout)", epilog="For more info see http://python.livinglogic.de/XIST_scripts_dtd2xsc.html")
