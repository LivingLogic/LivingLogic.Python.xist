...
expr10 returns [node]
	: a=atom { $node = $a.node; }
	| n=name '(' ')' { $node = ul4c.CallFunc(self.location, $n.text); }
	|
		n=name { $node = ul4c.CallFunc(self.location, $n.text); }
		'('
...
fragment
callarg returns [node]
	:
		n=name
		'='
		e=exprarg { $node = ($n.text, $e.node); }
	|
		'**'
		e=exprarg { $node = ($e.node,); }
	;
		...
		(
			...
			(
				...
				(
					/* No arguments */
					'('
					')' { callmeth = True; $node = ul4c.CallMeth(self.location, $n.text, $node); }
				|
					/* Positional argument */
					'(' { callmeth = True; $node = ul4c.CallMeth(self.location, $n.text, $node); }
					pa1=exprarg { $node.args.append($pa1.node); }
					(
						','
						pa2=exprarg { $node.args.append($pa2.node); }
					)*
					','?
					')'
				|
					/* Keyword arguments */
					'(' { callmeth = True; $node = ul4c.CallMethKeywords(self.location, $n.text, $node); }
					kwa1=callarg { $node.args.append($kwa1.node); }
					(
						','
						kwa2=callarg { $node.args.append($kwa2.node); }
					)*
					','?
					')'
				)
				...
				if not callmeth:
					$node = ul4c.GetAttr(self.location, $node, $n.text);
				}
		|
			/* Item/slice access */
			'['
...
expr7 returns [node]
	:
		...
		(
			...
			e2=expr8 { $node = cls(self.location, $node, $e2.node); }
		)*
	;
...
expr6 returns [node]
	:
		...
		(
			...
			e2=expr7 { $node = cls(self.location, $node, $e2.node) }
		)*
	;
...
expr5 returns [node]
	:
		...
		(
			...
			e2=expr6 { $node = cls(self.location, $node, $e2.node); }
		)*
	;
...
expr4 returns [node]
	:
		...
		(
			...
			'in'
			e2=expr5 { $node = cls(self.location, $node, $e2.node); }
		)?
	;
...
expr3 returns [node]
	:
		'not'
		e=expr4 { $node = ul4c.Not(self.location, $e.node); }
	|
		e=expr4 { $node = $e.node; }
	;
