...
def httpdate(dt):
	...
	``dt`` is a :class:`datetime.datetime` object. If ``dt.tzinfo`` is
	:const:`None` ``dt`` is assumed to be in the local timezone (using the
...
class Connect:
	...
	def __init__(self, connectstring=None, pool=None, retry=3, **kwargs):
		...
		``connectstring`` or ``pool`` (a database pool object) must be specified.
		``retry`` specifies how often to retry calling the wrapped function after
		a database exception. ``kwargs`` will be passed on to the
		:func:`connect` call.
		"""
...
class Call:
	...
	def __call__(self, *args, **kwargs):
		"""
		Call the procedure/function with the arguments ``args`` and ``kwargs``
		mapping Python function arguments to Oracle procedure/function arguments.
		On return from the procedure the ``c_out`` parameter is mapped to the
		CherryPy response body, and the parameters ``p_expires`` (the number of
		days from now), ``p_lastmodified`` (a date in UTC), ``p_mimetype``
		(a string), ``p_encoding`` (a string), ``p_etag`` (a string) and
