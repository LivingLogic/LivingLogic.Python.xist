...
class JavascriptMinifyAction(PipeAction):
	...
	def _outA(self):
		self.outstream.write(self.a)

	def _outB(self):
		self.outstream.write(self.b)
	...
	def _jsmin(self):
		...
		while self.a != "\000":
			if self.a == " ":
				self._action(1 if self.isalphanum(self.b) else 2)
			elif self.a == "\n":
				if self.b in "{[(+-":
					self._action(1)
				elif self.b == " ":
				...
				if self.b == " ":
					self._action(1 if self.isalphanum(self.a) else 3)
				elif self.b == "\n":
					if self.a in "}])+-\"'":
						self._action(1)
					else:
