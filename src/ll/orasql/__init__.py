...
class Constraint(Object):
	...
	def iternames(cls, connection, schema="user"):
		cursor = connection.cursor()
		if schema == "all":
			cursor.execute("select decode(owner, user, null, owner) as owner, constraint_name from all_constraints where constraint_type=:type order by owner, constraint_name", type=cls.constraint_type)
		else:
			cursor.execute("select null as owner, constraint_name from user_constraints where constraint_type=:type order by constraint_name", type=cls.constraint_type)
...
class Index(MixinNormalDates, Object):
	...
	def iternames(cls, connection, schema="user"):
		...
		cursor = connection.cursor()
		if schema=="all":
			cursor.execute("select decode(owner, user, null, owner) as owner, object_name from (select owner, object_name from all_objects where object_type = 'INDEX' minus select owner, constraint_name as object_name from all_constraints where constraint_type in ('U', 'P')) order by owner, object_name")
		else:
			cursor.execute("select null as owner, object_name from (select object_name from user_objects where object_type = 'INDEX' minus select constraint_name as object_name from user_constraints where constraint_type in ('U', 'P')) order by object_name")
		return ((row.object_name, row.owner) for row in cursor)
