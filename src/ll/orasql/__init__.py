...
class Index(MixinNormalDates, Object):
	...
	def createddl(self, connection=None, term=True):
		...
		if rec.index_type == "DOMAIN":
			code += " indextype is {}.{} parameters ('{}')".format(rec.ityp_owner, rec.ityp_name, rec.parameters.replace("'", "''"))
		if term:
			code += ";\n"
		else:
			code += "\n"
	...
	def iternames(cls, connection, owner=ALL):
		...
		cursor = connection.cursor()
		if owner is None:
			cursor.execute("select null as owner, index_name from (select index_name from user_indexes where index_type != 'LOB' minus select constraint_name as index_name from user_constraints where constraint_type in ('U', 'P')) where index_name not like 'BIN$%' order by index_name")
		elif owner is ALL:
			cursor.execute("select decode(owner, user, null, owner) as owner, index_name from (select owner, index_name from {0}_indexes where index_type != 'LOB' minus select owner, constraint_name as index_name from {0}_constraints where constraint_type in ('U', 'P')) where index_name not like 'BIN$%' order by owner, index_name".format(cursor.ddprefix()))
		else:
			cursor.execute("select decode(owner, user, null, owner) as owner, index_name from (select owner, index_name from {0}_indexes where index_type != 'LOB' and owner = :owner minus select owner, constraint_name as index_name from {0}_constraints where constraint_type in ('U', 'P') and owner = :owner) where index_name not like 'BIN$%' order by owner, index_name".format(cursor.ddprefix()), owner=owner)
