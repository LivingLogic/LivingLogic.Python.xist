...
class Versions:
	...
	def __init__(self):
		self.versions = [Version(version, date) for (version, date) in self.info]
...
class Version:
	...
	def output(self):
		...
		print()
		if self.files:
			widthfile = max(len(f.rest()) for f in self.files)
			widthtype = max(len(f.type or "") for f in self.files)
			widthsize = max(len("{:,}".format(f.size)) for f in self.files)
			print(".. tabularcolumns:: |l|l|r|")
			print()
			print(".. rst-class:: download")
			print()
			print("=" * widthfile, "=" * widthtype, "=" * widthsize)
			print("{:{}} {:{}} {:{}}".format("File", widthfile, "Type", widthtype, "Size", widthsize))
			print("=" * widthfile, "=" * widthtype, "=" * widthsize)
			for f in self.files:
				print("{:{}} {:{}} {:{},}".format(f.rest(), widthfile, f.type or "", widthtype, f.size, widthsize))
			print("=" * widthfile, "=" * widthtype, "=" * widthsize)
		else:
			print("(no files for this version)")
