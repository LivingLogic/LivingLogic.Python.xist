...
from ll.xist.ns.text import text
from ll.xist.ns.docbook import docbook
...
class doc(xsc.Namespace):
	...
	class function(base):
		...
		def convert_docbook(self, converter):
			return target.function(self.content)
	...
	def getdoc(cls, thing):
		...
		if not node.find(type=doc.par): # optimization: one paragraph docstrings don't need a <par> element.
			node = cls.par(node)

		refs = node.find(type=doc.pyref, subtype=1, searchchildren=1)
		if inspect.ismethod(thing):
			for ref in refs:
