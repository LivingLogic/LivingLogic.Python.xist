...
class css(xsc.Namespace):
	...
	class css(xsc.Element):
		...
		def publish(self, publisher):
			...
			imports = self.find(type=css.atimport)
			first = True
			for i in imports:
				if first:
					first = False
					...
					publisher.publish(u"\n")
				i.publish(publisher)
			content = self.find(type=(css.rule, css.atmedia), subtype=1)
	...
	class atmedia(xsc.Element):
		...
		def publish(self, publisher):
			...
			for i in imports:
				publisher.publish(u"\n\t")
				i.publish(publisher)
			content = self.find(type=css.rule, subtype=1)
			for child in content:
				publisher.publish(u"\n\t")
				child.publish(publisher)
			publisher.publish(u"\n}")
	...
	class rule(xsc.Element):
		...
		def publish(self, publisher):
			sels = self.find(type=css.sel)
			props = self.find(type=css.prop, subtype=1)
