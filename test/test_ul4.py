...
class TemplateJava:
	...
	def run(self, data):
		...
		try:
			...
			result = subprocess.run("./gradlew -q --console=plain execute_ul4", input=dump, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
		finally:
			os.chdir(currentdir)
...
def test_function_format_float(T):
	...
	assert t2.renders(data=42.0, fmt="06.2f", lang="en") == "042.00"
