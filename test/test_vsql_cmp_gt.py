...
def test_none_bool1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_bool", where="r.identifier == 'none'")
...
def test_none_int1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_int", where="r.identifier == 'none'")
...
def test_none_number1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_number", where="r.identifier == 'none'")
...
def test_none_str1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_str", where="r.identifier == 'none'")
...
def test_none_date1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_date", where="r.identifier == 'none'")
...
def test_none_datetime1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_datetime", where="r.identifier == 'none'")
...
def test_none_color1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_color", where="r.identifier == 'none'")
...
def test_none_datedelta1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_datedelta", where="r.identifier == 'none'")
...
def test_none_datetimedelta1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_datetimedelta", where="r.identifier == 'none'")
...
def test_none_monthdelta1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_monthdelta", where="r.identifier == 'none'")
...
def test_bool_bool6(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_bool > True", where="r.identifier == 'bool_false'")
...
def test_bool_bool8(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_bool > False", where="r.identifier == 'bool_true'")
...
def test_bool_int5(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_bool > 1", where="r.identifier == 'bool_false'")
...
def test_bool_int7(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_bool > 0", where="r.identifier == 'bool_true'")
...
def test_bool_number4(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_bool > 0.0", where="r.identifier == 'bool_false'")


def test_bool_number5(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_bool > 1.0", where="r.identifier == 'bool_false'")
...
def test_bool_number7(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_bool > 0.0", where="r.identifier == 'bool_true'")


def test_bool_number8(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_bool > 1.0", where="r.identifier == 'bool_true'")
...
def test_int_int3(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_int > 1775", where="r.identifier == 'int'")


def test_int_int4(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_int > 1777", where="r.identifier == 'int'")


def test_int_int5(vsql_db, vsql_data):
	assert vsql_db.expr("1777 > r.v_int", where="r.identifier == 'int'")
...
def test_number_int3(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_number > 1", where="r.identifier == 'number'")


def test_number_int4(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_number > 73", where="r.identifier == 'number'")
...
def test_number_number3(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_number > 1.0", where="r.identifier == 'number'")


def test_number_number4(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_number > 73.0", where="r.identifier == 'number'")


def test_str_str(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_str > 'abc'", where="r.identifier == 'str'")


def test_date_date(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_date > @(2000-02-28)", where="r.identifier == 'date'")


def test_datetime_datetime(vsql_db, vsql_data):
	assert vsql_db.expr("r.v_datetime > @(2000-02-28T23:59:59)", where="r.identifier == 'datetime'")


def test_datedelta_datedelta(vsql_db, vsql_data):
	assert vsql_db.expr("days(2) > days(1)")


def test_datetimedelta_datetimedelta(vsql_db, vsql_data):
	assert vsql_db.expr("hours(2) > hours(1)")
...
def test_intlist_intlist4(vsql_db, vsql_data):
	assert not vsql_db.expr("[1, 2] > [1, 2]")
...
def test_numberlist_numberlist2(vsql_db, vsql_data):
	assert vsql_db.expr("[1.5, 3.5] > [1.5, 2.5]")
...
def test_numberlist_numberlist4(vsql_db, vsql_data):
	assert not vsql_db.expr("[1.5, 2.5] > [1.5, 2.5]")
...
def test_strlist_strlist2(vsql_db, vsql_data):
	assert vsql_db.expr("['foo', 'baz'] > ['foo', 'bar']")
...
def test_strlist_strlist4(vsql_db, vsql_data):
	assert not vsql_db.expr("['foo', 'bar'] > ['foo', 'bar']")
...
def test_datelist_datelist2(vsql_db, vsql_data):
	assert vsql_db.expr("[@(2000-02-29), @(2000-03-02)] > [@(2000-02-29), @(2000-03-01)]")
...
def test_datelist_datelist4(vsql_db, vsql_data):
	assert not vsql_db.expr("[@(2000-02-29), @(2000-03-01)] > [@(2000-02-29), @(2000-03-01)]")
...
def test_datetimelist_datetimelist2(vsql_db, vsql_data):
	assert vsql_db.expr("[@(2000-02-29T12:34:56), @(2000-03-02T12:34:56)] > [@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)]")
...
def test_datetimelist_datetimelist4(vsql_db, vsql_data):
	assert not vsql_db.expr("[@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)] > [@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)]")
...
def test_strlist_nulllist2(vsql_db, vsql_data):
	assert vsql_db.expr("['gurk'] > [None]") is None
...
def test_null_none(vsql_db, vsql_data):
	...
	assert not vsql_db.expr("[None, None][0] > None")
...
def test_clob_none1(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_clob > None", where="r.identifier == 'none'")
...
def test_none_clob1(vsql_db, vsql_data):
	assert not vsql_db.expr("None > r.v_clob", where="r.identifier == 'none'")
...
def test_clob_clob1(vsql_db, vsql_data):
	assert not vsql_db.expr("r.v_clob > r.v_clob", where="r.identifier == 'shortclob'")
