...
def test_bool_none2(db, vsql_data):
	assert not expr(db, "r.v_bool <= None", where="r.identifier == 'bool_false'")
...
def test_intlist_none(db, vsql_data):
	assert not expr(db, "[1, 2] <= None")


def test_numberlist_none(db, vsql_data):
	assert not expr(db, "[1.2, 3.4] <= None")


def test_strlist_none(db, vsql_data):
	assert not expr(db, "['foo', 'bar'] <= None")
...
def test_none_bool1(db, vsql_data):
	assert expr(db, "None <= r.v_bool", where="r.identifier == 'none'")


def test_none_bool2(db, vsql_data):
	assert expr(db, "None <= r.v_bool", where="r.identifier == 'bool_false'")


def test_none_bool3(db, vsql_data):
	assert expr(db, "None <= r.v_bool", where="r.identifier == 'bool_true'")


def test_none_int1(db, vsql_data):
	assert expr(db, "None <= r.v_int", where="r.identifier == 'none'")


def test_none_int2(db, vsql_data):
	assert expr(db, "None <= r.v_int", where="r.identifier == 'int'")


def test_none_number1(db, vsql_data):
	assert expr(db, "None <= r.v_number", where="r.identifier == 'none'")


def test_none_number2(db, vsql_data):
	assert expr(db, "None <= r.v_number", where="r.identifier == 'number'")


def test_none_str1(db, vsql_data):
	assert expr(db, "None <= r.v_str", where="r.identifier == 'none'")


def test_none_str2(db, vsql_data):
	assert expr(db, "None <= r.v_str", where="r.identifier == 'str'")


def test_none_date1(db, vsql_data):
	assert expr(db, "None <= r.v_date", where="r.identifier == 'none'")


def test_none_date2(db, vsql_data):
	assert expr(db, "None <= r.v_date", where="r.identifier == 'date'")


def test_none_datetime1(db, vsql_data):
	assert expr(db, "None <= r.v_datetime", where="r.identifier == 'none'")


def test_none_datetime2(db, vsql_data):
	assert expr(db, "None <= r.v_datetime", where="r.identifier == 'datetime'")


def test_none_color1(db, vsql_data):
	assert expr(db, "None <= r.v_color", where="r.identifier == 'none'")


def test_none_color2(db, vsql_data):
	assert expr(db, "None <= r.v_color", where="r.identifier == 'color'")


def test_none_datedelta1(db, vsql_data):
	assert expr(db, "None <= r.v_datedelta", where="r.identifier == 'none'")


def test_none_datedelta2(db, vsql_data):
	assert expr(db, "None <= r.v_datedelta", where="r.identifier == 'datedelta'")


def test_none_datetimedelta1(db, vsql_data):
	assert expr(db, "None <= r.v_datetimedelta", where="r.identifier == 'none'")


def test_none_datetimedelta2(db, vsql_data):
	assert expr(db, "None <= r.v_datetimedelta", where="r.identifier == 'datetimedelta'")


def test_none_monthdelta1(db, vsql_data):
	assert expr(db, "None <= r.v_monthdelta", where="r.identifier == 'none'")


def test_none_monthdelta2(db, vsql_data):
	assert expr(db, "None <= r.v_monthdelta", where="r.identifier == 'monthdelta'")


def test_none_intlist(db, vsql_data):
	assert expr(db, "None <= [1, 2]")


def test_none_numberlist(db, vsql_data):
	assert expr(db, "None <= [1.2, 3.4]")


def test_none_strlist(db, vsql_data):
	assert expr(db, "None <= ['foo', 'bar']")
...
def test_bool_bool2(db, vsql_data):
	assert expr(db, "r.v_bool <= False", where="r.identifier == 'none'")


def test_bool_bool3(db, vsql_data):
	assert expr(db, "r.v_bool <= True", where="r.identifier == 'none'")
...
def test_bool_bool6(db, vsql_data):
	assert expr(db, "r.v_bool <= True", where="r.identifier == 'bool_false'")
...
def test_bool_bool8(db, vsql_data):
	assert not expr(db, "r.v_bool <= False", where="r.identifier == 'bool_true'")
...
def test_bool_int2(db, vsql_data):
	assert expr(db, "r.v_bool <= -1", where="r.identifier == 'none'")
...
def test_bool_int5(db, vsql_data):
	assert expr(db, "r.v_bool <= 1", where="r.identifier == 'bool_false'")
...
def test_bool_int7(db, vsql_data):
	assert not expr(db, "r.v_bool <= 0", where="r.identifier == 'bool_true'")
...
def test_bool_number2(db, vsql_data):
	assert expr(db, "r.v_bool <= -1.0", where="r.identifier == 'none'")
...
def test_bool_number4(db, vsql_data):
	assert expr(db, "r.v_bool <= 0.0", where="r.identifier == 'bool_false'")


def test_bool_number5(db, vsql_data):
	assert expr(db, "r.v_bool <= 1.0", where="r.identifier == 'bool_false'")
...
def test_bool_number7(db, vsql_data):
	assert not expr(db, "r.v_bool <= 0.0", where="r.identifier == 'bool_true'")


def test_bool_number8(db, vsql_data):
	assert expr(db, "r.v_bool <= 1.0", where="r.identifier == 'bool_true'")
...
def test_int_bool2(db, vsql_data):
	assert expr(db, "r.v_int <= False", where="r.identifier == 'none'")


def test_int_bool3(db, vsql_data):
	assert expr(db, "r.v_int <= True", where="r.identifier == 'none'")
...
def test_int_int2(db, vsql_data):
	assert expr(db, "r.v_int <= 1", where="r.identifier == 'none'")


def test_int_int3(db, vsql_data):
	assert not expr(db, "r.v_int <= 1775", where="r.identifier == 'int'")


def test_int_int4(db, vsql_data):
	assert expr(db, "r.v_int <= 1777", where="r.identifier == 'int'")


def test_int_int5(db, vsql_data):
	assert not expr(db, "1776 <= r.v_int", where="r.identifier == 'none'")
...
def test_number_bool2(db, vsql_data):
	assert expr(db, "r.v_number <= False", where="r.identifier == 'none'")
...
def test_number_int2(db, vsql_data):
	assert expr(db, "r.v_number <= 1", where="r.identifier == 'none'")


def test_number_int3(db, vsql_data):
	assert not expr(db, "r.v_number <= 1", where="r.identifier == 'number'")


def test_number_int4(db, vsql_data):
	assert expr(db, "r.v_number <= 73", where="r.identifier == 'number'")
...
def test_number_number2(db, vsql_data):
	assert expr(db, "r.v_number <= 1.0", where="r.identifier == 'none'")


def test_number_number3(db, vsql_data):
	assert not expr(db, "r.v_number <= 1.0", where="r.identifier == 'number'")


def test_number_number4(db, vsql_data):
	assert expr(db, "r.v_number <= 73.0", where="r.identifier == 'number'")


def test_str_str1(db, vsql_data):
	assert expr(db, "'abc' <= r.v_str", where="r.identifier == 'str'")


def test_str_str2(db, vsql_data):
	assert expr(db, "r.v_str <= r.v_str", where="r.identifier == 'str'")


def test_date_date(db, vsql_data):
	assert expr(db, "@(2000-02-28) <= r.v_date", where="r.identifier == 'date'")


def test_datetime_datetime(db, vsql_data):
	assert expr(db, "@(2000-02-28T23:59:59) <= r.v_datetime", where="r.identifier == 'datetime'")


def test_datedelta_datedelta1(db, vsql_data):
	assert expr(db, "days(1) <= days(2)")


def test_datedelta_datedelta2(db, vsql_data):
	assert expr(db, "days(1) <= days(1)")


def test_datetimedelta_datetimedelta1(db, vsql_data):
	assert expr(db, "hours(1) <= hours(2)")


def test_datetimedelta_datetimedelta2(db, vsql_data):
	assert expr(db, "hours(1) <= hours(1)")
...
def test_numberlist_numberlist2(db, vsql_data):
	assert expr(db, "[1.5, 2.5] <= [1.5, 3.5]")
...
def test_numberlist_numberlist4(db, vsql_data):
	assert expr(db, "[1.5, 2.5] <= [1.5, 2.5]")
...
def test_strlist_strlist2(db, vsql_data):
	assert expr(db, "['foo', 'bar'] <= ['foo', 'baz']")
...
def test_strlist_strlist4(db, vsql_data):
	assert expr(db, "['foo', 'bar'] <= ['foo', 'bar']")
...
def test_datelist_datelist2(db, vsql_data):
	assert expr(db, "[@(2000-02-29), @(2000-03-01)] <= [@(2000-02-29), @(2000-03-02)]")
...
def test_datelist_datelist4(db, vsql_data):
	assert expr(db, "[@(2000-02-29), @(2000-03-01)] <= [@(2000-02-29), @(2000-03-01)]")
...
def test_datetimelist_datetimelist2(db, vsql_data):
	assert expr(db, "[@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)] <= [@(2000-02-29T12:34:56), @(2000-03-02T12:34:56)]")
...
def test_datetimelist_datetimelist4(db, vsql_data):
	assert expr(db, "[@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)] <= [@(2000-02-29T12:34:56), @(2000-03-01T12:34:56)]")
